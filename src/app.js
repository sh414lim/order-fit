...
function vendors() { const rows = [['푸른식자재','₩1,840,000','24건','양파, 대파, 감자'],['한성정육','₩1,250,000','12건','한우 등심, 삼겹살'],['그린패키지','₩698,200','9건','식용유, 포장용기']]; return `<article class="panel"><div class="panel-heading"><div><p class="eyebrow">이번 달</p><h2>업체별 발주 현황</h2></div><button class="secondary-button">업체 추가</button></div><div class="table-wrap"><table><thead><tr><th>업체명</th><th>누적 발주금액</th><th>발주 횟수</th><th>주요 품목</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${r[0]}</strong></td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td></tr>`).join('')}</tbody></table></div></article>`; }
function items() { return `<section class="metrics">${card('평균 단가 상승 품목','8개','전주 대비 2개 증가','attention')}${card('표준화 필요 품목','4개','원문 품목명 연결 필요')}${card('이번 달 구매 품목','86개','총 1,922개 수량')}</section><article class="panel"><div class="panel-heading"><div><p class="eyebrow">품목 마스터</p><h2>구매 및 단가 현황</h2></div><button class="secondary-button">품목 추가</button></div><div class="table-wrap"><table><thead><tr><th>품목</th><th>카테고리</th><th>평균 단가</th><th>최근 단가</th><th>주요 업체</th></tr></thead><tbody><tr><td><strong>양파</strong></td><td>채소</td><td>₩1,600/kg</td><td class="good">₩1,520/kg ↓</td><td>푸른식자재</td></tr><tr><td><strong>한우 등심</strong></td><td>육류</td><td>₩42,000/kg</td><td class="attention">₩47,000/kg ↑</td><td>한성정육</td></tr><tr><td><strong>식용유</strong></td><td>소스·유지</td><td>₩2,880/L</td><td>₩2,850/L</td><td>그린패키지</td></tr></tbody></table></div></article>`; }
...
document.getElementById('start-ocr').addEventListener('click', event => { const file = document.getElementById('receipt-file').files[0]; if (!file) return; event.preventDefault(); state.receipts.unshift({ id: Date.now(), vendor: 'OCR 분석 중', date: new Date().toLocaleDateString('ko-KR').replace(/ /g, ''), total: 0, status: '검토 필요', lines: 5 }); persist(); document.getElementById('receipt-dialog').close(); render('receipts'); });
render();
